...
<section class="hero">
    <div class="contenedor">
        ...
        <h1 style="margin-top:8px">Colección Talla {{ $talla }} 👶</h1>
        <p>Todos los productos disponibles en talla {{ $talla }}. Agrégalos al carrito directo aquí.</p>
        <div class="pills" style="margin-top:14px">
            @foreach(['S','M','L','XL','XXL','XXXL'] as $t)
                <a class="pill-i" href="{{ route('store.talla', $t) }}"
                   style="{{ $t === $talla ? 'background:var(--azul);color:#fff;border-color:var(--azul)' : '' }}">Talla {{ $t }}</a>
            @endforeach
        </div>
    </div>
</section>
...
<main class="contenedor">
    @if(count($items) === 0)
        <div class="sg-none" style="margin:24px 0">Por ahora no hay productos en talla {{ $talla }}.
    ...
    @else
        <div class="grid" style="padding-top:24px">
            @foreach($items as $it)
                @php $p = $it['product']; $s = $it['size']; @endphp
                <div class="pcard" x-data="{ cantidad: 1 }" style="cursor:default">
                    <a class="img" href="{{ route('store.show', $p) }}">@if($p->oferta)<span class="oferta-bubble">{{ $p->oferta }}</span>@endif<img src="{{ $p->imageUrl() }}" alt="{{ $p->name }}" loading="lazy"></a>
                    <div class="body">
                        <div class="marca">{{ $p->brand }}</div>
                        <a class="nom" href="{{ route('store.show', $p) }}" style="color:inherit">{{ $p->name }}</a>
                        ...
                        <div style="display:flex;gap:8px;align-items:center;margin-top:8px">
                            ...
                            <button class="btn btn-azul" style="flex:1"
                                @click="$store.cart.agregar({ id: {{ $p->id }}, talla: @js($s->size), cantidad: Math.max(1, cantidad), nombre: @js($p->name), imagen: @js($p->imageUrl()), precio: {{ (float) $s->price }}, combo: @js($s->combo_qty ? ['cantidad' => (int) $s->combo_qty, 'precio' => (float) $s->combo_price] : null) })">
                                Agregar 🛒
                            </button>
                        </div>
                    </div>
